...
class FrozenDict(Mapping[K, V]):
  ...
  def __init__(
      self, *args: Any, __unsafe_skip_copy__: bool = False, **kwargs: Any
  ):  # pylint: disable=invalid-name
    # make sure the dict is as
    xs: Dict[Any, Any] = dict(*args, **kwargs)
  ...
  def __getitem__(self, key: K) -> Union["FrozenDict[V, Any]", V]:
    v = self._dict[key]
  ...
  def pretty_repr(self, num_spaces: int = 4) -> str:
    ...
    def pretty_dict(x: Dict[Any, Any]) -> str:
  ...
  def values(self) -> FrozenValuesView:
    return FrozenValuesView(self)

  def items(self) -> Iterable[Tuple[K, Union["FrozenDict[V, Any]", V]]]:
    for key in self._dict:
      yield (key, self[key])

  def pop(self, key: K) -> Tuple["FrozenDict[K, V]", Union["FrozenDict[V, Any]", V]]:
  ...
  def unfreeze(self) -> Dict[Any, Any]:
  ...
  @classmethod
  def tree_unflatten(
      cls, keys: Iterable[K], values: Iterable[V]
  ) -> "FrozenDict[K, V]":
...
def _prepare_freeze(xs: Union[FrozenDict[Any, Any], Dict[Any, Any]]) -> Dict[Any, Any]:
...
def unfreeze(
  x: Union[FrozenDict[K, V], Dict[str, Any]]
) -> Union[Dict[K, V], Dict[str, Any]]:
...
def pop(
  x: Union[FrozenDict[Any, Any], Dict[str, Any]], key: str
) -> Tuple[Union[FrozenDict[Any, Any], Dict[str, Any]], Any]:
...
def pretty_repr(
  x: Union[FrozenDict[Any, Any], Dict[Any, Any]], num_spaces: int = 4
) -> str:
  """Returns an indented representation of the nested dictionary.
  ...
  def pretty_dict(x: Dict[Any, Any]):
    if not isinstance(x, dict):
      return repr(x)
    rep = ""
    for key, val in x.items():
...
def _frozen_dict_state_dict(xs: FrozenDict[K, Any]) -> Dict[K, Any]:
  return {key: serialization.to_state_dict(value) for key, value in xs.items()}


def _restore_frozen_dict(
  xs: Dict[str, Any], states: Dict[str, Any]
) -> FrozenDict[str, Any]:
